...
const MoiveDetails = ({movieDetails, movie, media_type}) => {
	...
	const date = (new Date(movieDetails?.release_date || movieDetails?.first_air_date)).toLocaleDateString('en-GB')
	return (
		<>
			<div className="flex flex-col gap-y-5 row-span-3">
				<h1 className="text-6xl">{movieDetails?.title || movieDetails?.original_title || movieDetails?.name}</h1>
				...
				<h1 className="text-xl flex gap-x-4 text-orange-400">{movieDetails?.status} - {date} <span
					...
					{media_type === "movie" ?
						...
						<div>
							...
							&nbsp;
							{/*{movieDetails?.number_of_seasons?.length &&*/}
								<span>{movieDetails?.number_of_seasons?.length <= 1 ? movieDetails?.number_of_seasons + " Season" : movieDetails?.number_of_seasons + " Seasons"}</span>
						</div>
					}
				</h1>
